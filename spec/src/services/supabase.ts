...
import { RegistroPonto, SupabaseConfig } from '../types';
...
-- CARGA DE COLABORADORES INICIAIS
...
-- POLÍTICAS RLS PARA ACESSO VIA REST API
...
/**
 * Testa a conexão com o Supabase informando URL e Anon Key
 */
export async function testarConexaoSupabase(url: string, anonKey: string): Promise<{ sucesso: boolean; mensagem: string }> {
  try {
    if (!url.startsWith('https://')) {
      return { sucesso: false, mensagem: 'A URL do Supabase deve iniciar com https://' };
    }
    const client = createClient(url, anonKey);
    // Tenta uma query de health check básica
    const { error } = await client.from('colaboradores').select('count', { count: 'exact', head: true });
    if (error && error.code !== 'PGRST116') {
      // Se a tabela ainda não foi criada, informa que conectou mas faltam tabelas
      if (error.code === '42P01') {
        return {
          sucesso: true,
          mensagem: 'Conexão estabelecida com sucesso! (Observação: Execute o script SQL para criar as tabelas).',
        };
      }
      return { sucesso: false, mensagem: `Falha na autenticação: ${error.message}` };
    }
    return { sucesso: true, mensagem: 'Conectado com sucesso ao Supabase!' };
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : 'Erro desconhecido ao testar conexão';
    return { sucesso: false, mensagem: msg };
  }
}

/**
 * RNF-01: Sincronização automática em background de registros locais para o Supabase
 */
export async function sincronizarRegistrosPendentes(): Promise<{ totalSincronizados: number }> {
  const config = await dbService.getSupabaseConfig();
  if (!config.conectado || !config.url || !config.anonKey) {
    return { totalSincronizados: 0 };
  }

  const client = getSupabaseInstance(config);
  if (!client) return { totalSincronizados: 0 };

  const naoSincronizados = await dbService.getRegistrosNaoSincronizados();
  if (naoSincronizados.length === 0) return { totalSincronizados: 0 };
  ...
  for (const reg of naoSincronizados) {
    try {
      const payload = {
        ...
        colaborador_nome: reg.colaboradorNome,
        colaborador_pis: reg.colaboradorPis,
        colaborador_cargo: reg.colaboradorCargo,
        departamento: reg.departamento,
        turno_nome: reg.turnoNome,
        tipo_evento: reg.tipoEvento,
        timestamp_utc: reg.timestampUtc,
        horario_local: reg.horarioLocal,
        data_iso: reg.dataIso,
        hora_iso: reg.horaIso,
        foto_capturada_url: reg.fotoCapturadaUrl || null,
        metodo_autenticacao: reg.metodoAutenticacao,
        dentro_tolerancia: reg.dentroTolerancia,
        minutos_desvio: reg.minutosDesvio,
        justificativa: reg.justificativa || null,
        comprovante_impresso: reg.comprovanteImpresso,
        comprovante_email_enviado: reg.comprovanteEmailEnviado,
        email_destinatario: reg.emailDestinatario || null,
        hash_integridade: reg.hashIntegridade,
      };

      const { error } = await client.from('registros_ponto').upsert(payload);
      if (!error) {
        await dbService.marcarRegistroSincronizado(reg.id);
        count++;
      } else {
        console.warn('Erro ao sincronizar registro específico:', error);
      }
    } catch (err) {
      console.error('Falha de rede ao sincronizar:', err);
      break;
    }
  }

  return { totalSincronizados: count };
}
